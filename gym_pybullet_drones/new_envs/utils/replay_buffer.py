...
class ReplayBuffer(object):
    def __init__(self, args):
        ...
        self.buffer_rgb_next_n = np.empty((self.buffer_size, self.N_drones, height, width, channels), dtype=np.uint8)
    ...
    def store_transition(self, rgb_n, obs_n, a_n, r_n, rgb_next_n, obs_next_n, done_n):
        self.buffer_rgb_n[self.count] = rgb_n
        self.buffer_obs_n[self.count] = obs_n
        self.buffer_a_n[self.count] = a_n
        self.buffer_r_n[self.count] = r_n
        self.buffer_rgb_next_n[self.count] = rgb_next_n
    ...
    def sample(self):
        ...
        batch_rgb_n = torch.tensor(self.buffer_rgb_n[index], dtype=torch.float) / 255.0
        ...
        batch_rgb_next_n = torch.tensor(self.buffer_rgb_next_n[index], dtype=torch.float) / 255.0
